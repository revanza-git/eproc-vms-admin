...
class Filter
{
			...
			foreach ($row['filter'] as $key => $value) {
				...
				switch ($value['type']) {
					...
					case 'dropdown':
						$html .= '<div class="groupFieldBlock"><label class="title">' . $value['label'] . '</label>';
						// print_r($field);die;
						$param = ($field[3] == '') ? '' : $field[3];
						$data_dropdown = $this->$field[2]($param);
						// print_r($data_dropdown);die;
						$html .= '<div class="groupFieldInput" name="filter[' . $value['table'] . '][]" >' .
	...
	public function get_for_barang($category)
	{
		return array(
			'1' 	=> 'Divisi Operasi',
			'10' 	=> 'Divisi Reliability&Quality',
			'18' 	=> 'Departemen Layanan Umum',
			'15' 	=> 'Seksi Sistem Informasi',
			'3' 	=> 'Departemen Sekretaris Perusahaan',
			'5' 	=> 'Departemen HSSE'
		);
	}

	public function get_for_jasa($value = '')
	{
		return array(
			'6' => 'Jasa Konsultasi',
			'7' => 'Jasa Konstruksi',
	...
	{
		$arr = $this->CI->db->select('id,name')->where('(id != 1)')->get('tb_dpt_type')->result_array();
		$result = array();
		foreach ($arr as $key => $row) {
			$result[$row['id']] = $row['name'];
		}
	...
	public function get_field($array = array())
	{
		$a =  array(
			...
			array(
				...
				'filter' =>	array(
					array('table' => 'ms_ijin_usaha|id_dpt_type|get_filter_dpt_type', 'type' => 'dropdown', 'label' => 'Tipe DPT'),
