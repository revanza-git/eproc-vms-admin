...
<div class="tableWrapper">
	...
	<a style="background: #2f3640 !important; border-bottom: 2px #718093 solid !important;" href="<?php echo site_url('blacklist/export_excel/'.$id_blacklist);?>" class="btnBlue"><i class="fa fa-download"></i> Export</a>
		<form method="POST">
	...
	<table class="tableData remark">
		...
		<thead>
			...
			<tr>
				...
				<td><a href="?<?php echo $this->utility->generateLink('sort','desc')?>&sort=<?php echo ($sort['id_vendor'] == 'asc') ? 'desc' : 'asc'; ?>&by=id_vendor">Nama<i class="fa fa-sort-<?php echo ($sort['id_vendor'] == 'asc') ? 'desc' : 'asc'; ?>"></i></a></td>

				

				<td><a href="?<?php echo $this->utility->generateLink('sort','desc')?>&sort=<?php echo ($sort['point'] == 'asc') ? 'desc' : 'asc'; ?>&by=point">Skor terakhir<i class="fa fa-sort-<?php echo ($sort['point'] == 'asc') ? 'desc' : 'asc'; ?>"></i></a></td>
				...
				<td><a href="?<?php echo $this->utility->generateLink('sort','desc')?>&sort=<?php echo ($sort['total_bl'] == 'asc') ? 'desc' : 'asc'; ?>&by=total_bl">Total<i class="fa fa-sort-<?php echo ($sort['total_bl'] == 'asc') ? 'desc' : 'asc'; ?>"></i></a></td>
				<td>File</td>
		...
		?> 
				...
				<tr style="<?php echo $yellow;?>">
					...
					<td class="actionBlock">
							<a href="<?php echo site_url('vendor/dpt_print/'.$value['id_vendor'])?>" class="editBtn">
							<i class="fa fa-print"></i>&nbsp;Print Data
 						</a>
 						<a href="<?php echo site_url('/approval/administrasi/'.$value['id_vendor'])?>" class="editBtn">
						...
						<?php 
							...
							if($this->session->userdata('admin')['id_role']==8){
								...
								if($value['need_approve_bl']==1){
									...
									<a href="<?php echo site_url('blacklist/approve_aktif/'.$value['id_tr'])?>"  class="alertApprove"><i class="fa fa-check-square-o"></i>Setuju Aktifkan kembali</a>

									<?php } else{ ?>

									<a href="<?php echo site_url('blacklist/approve_form/'.$value['id_vendor'].'/'.$value['id_tr'].'/'.$value['id_blacklist'])?>" class="alertApprove"><i class="fa fa-check-square-o"></i>&nbsp;Setujui Masuk <?php echo $value['blacklist_val']?></a>
							...
							if($this->session->userdata('admin')['id_role']==1 && $value['need_approve_bl'] == 0){
								...
								| <a href="<?php echo site_url('blacklist/aktif/'.$value['id_tr'])?>" ><i class="fa fa-check-square-o"></i>Aktifkan</a>
