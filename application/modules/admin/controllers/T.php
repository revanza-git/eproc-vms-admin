...
class T extends CI_Controller{
	...
	public function test()
	{
		$return ='<html>
				...
				<body>
					<style type="text/css" media="screen">
						...
						div#first-page .certificate .certificateTitle h2{
							font-size: 30px;
									...
									<h2>PT DEKODR INDONESIA</h2>
											<table>
												...
												<tr>
														...
														<td>Alamat</td>
														<td>:</td>
														<td>KOMPLEK PERKANTORAN PT. PERUSAHAAN GAS NEGASA (PERSERO) Tbk, GEDUNG B LANTAI 3,4,5 JALAN K.H ZAINUL ARIFIN NO.20 KEL.KRUKUT, KEC.TAMAN SARI, KOTA ADMINISTRASI JAKARTA BARAT </td>
		...
		// $this->load->view('admin/output_report');
		...
		// $this->dompdf->stream('test.pdf',array('Attachment' => 1));

		// echo $return;die;

			// $this->pdfgenerator->setPaper('A4', 'potrait');
		 //    $this->pdfgenerator->filename = "Sertifikat-DEKODR INDONESIA.pdf";
		 //    $this->pdfgenerator->load_view($return);
		// return $this->pdfgenerator->generate($return,'Sertifikat-DEKODR INDONESIA','','A4','landscape');
		...
		$dompdf->stream("sertifikat - DEKODR.pdf",array('Attachment' => 1));
	}
	function change_no(){
		if($this->vm->change_no($this->input->get('id'),$this->input->get('no'))){
			echo 1;
			$this->session->set_flashdata('msgSuccess','<p class="msgSuccess">Sukses 

mengubah data!</p>');
		}else{
			echo 0;
			$this->session->set_flashdata('msgSuccess','<p class="msgError">Gagal mengubah 
...
data!</p>');
	...
	function dpt($id = ""){
		...
		$data['value_k3']		= $this->km->get_penilaian_value($id,$data
...
['csms_file']['id']);
		...
		$logo = 'http://dekodr.co.id/login-regas-logo.jpg';
		...
		foreach ($data['surat_izin'] as $jenis => $valueTotal) {
			...
			$jenis = ($jenis=='siup') ? "SIUP" : (($jenis=='asosiasi' ? "Asosiasi" : 

($jenis=='ijin_lain' ? "Izin Lain" : "&nbsp;"))) ;

			$izin_usaha		.= '<table cellpadding="2" cellspacing="0" width="95%" 

border="1" class="std-table">';
			$izin_usaha		.= 		'<tr>';
			$izin_usaha		.=			'<th style="background : 

#c0392b; color : #eee" height="20" colspan="5">'.$jenis.'</th>';
			$izin_usaha		.=		'</tr>';

			$izin_usaha		.=		'<tr>';
			$izin_usaha		.=			'<th style="background : 

#e74c3c; color : #eee"; height="20";>No.</th>';
			$izin_usaha		.=			'<th style="background : 

#e74c3c; color : #eee"; height="20";>Tanggal</th>';
			$izin_usaha		.=			'<th style="background : 

#e74c3c; color : #eee"; height="20";>Kualifikasi</th>';
			$izin_usaha		.=			'<th style="background : 

#e74c3c; color : #eee"; height="20";>Bidang/Sub-Bidang</th>';
			$izin_usaha		.=			'<th style="background : 

#e74c3c; color : #eee"; height="20";>Masa Berlaku</th>';
			$izin_usaha		.=		'</tr>';
...
'<td>'.default_date(date("d-m-Y", strtotime($value['issue_date']))).'</td>';
					$izin_usaha		.=			'<td>'.$value
...
'</ul>';
					$izin_usaha		.=			'</td>';
					$expire 		 = ($value['expire_date']=="lifetime") 

? "Seumur Hidup" : default_date($value['expire_date']);
					$izin_usaha		.=			
...
'<td>'.$expire.'</td>';
		...
		$rekap_k3 	= '<table class="table-separator" cellpadding=0 cellspacing=0 style=" 
...
border-collapse: collapse;">
		...
		foreach($data['evaluasi'] as $key_ms => $value_ms){
			if(isset($value_ms)){
				$rekap_k3		.= '<tr>';
				$rekap_k3		.=		'<td style="text-align:center; 

vertical-align:top;">&nbsp;'.$nomor_rekap.'&nbsp;</td>';
				$rekap_k3		.=		'<td colspan=2>'.$data
...
['ms_quest'][$key_ms].'</td>';
				...
				foreach($value_ms as $key_ev => $val_ev){
					$subtotal += isset($data['value_k3'][$key_ev]) ? $data

['value_k3'][$key_ev] : NULL;
				}
				$total_sub = round($subtotal / $total_data,2);
				$total +=$total_sub;
				$rekap_k3		.= '	<td 

class="nilai"><i>'.$total_sub.'</i></td>';
				$rekap_k3		.= '</tr>';
			}
...
style="border:none;"></td>';
				$rekap_k3		.= 		'<td style="border:none;" 

width="300px"></td>';
				$rekap_k3		.= 		'<td style="text-

align:center;">TOTAL</td>';
				$rekap_k3		.= 		'<td 

class="nilai"><b><i>'.$total.'</i></b></td>';
				$rekap_k3		.= '</tr>';
		$rekap_k3	.= '</table>';
...
cellpadding="0" cellspacing="0" style="margin: 0 auto; border-collapse: collapse;">
									<thead>
										<tr>
											<td>
											</td>
...
style="width: 5%; max-width: 10%;">
		...
		foreach($data['evaluasi'] as $key_ms => $value_ms){
			if(isset($value_ms)){
				...
				$csms_k3		.=	'<td colspan=2><b>Bagian '.$key_ms.' - 

 '.$data['ms_quest'][$key_ms].'</b></td>';
				...
				$total_data = count($value_ms);
				foreach($value_ms as $key_ev => $val_ev){
					$csms_k3	.= '<tr class="evalQuest" style="page-break-

inside: avoid;">';
					$csms_k3	.=	'<td class="textQuestLv1">'.$data

['evaluasi_list'][$key_ev]['name'].'</td>';
					$csms_k3	.=	'<b>'.$this->utility->generate_checked_k3($key_ev,$data['evaluasi_list'],isset($data['value_k3'][$key_ev]) ? $data

['value_k3'][$key_ev] : NULL).'</b>';
					$csms_k3	.= '</tr>';
...
(($total_data==1) ? 1 :'1/'.$total_data).'
		...
		if ($data['get_csms']['value']!="") {
			/*$k3_data = '<div class="page second-page" style="margin: 0 auto; page-break

-inside: avoid;">
							<div class="rekapContainer">
								<h2 align=center>REKAP NILAI HASIL 

PRAKUALIFIKASI DOKUMEN CSMS</h2>
								<p class="nomorSertifikat">No. 

'.$this->session->userdata('nomorCSMS').'</p>
								<table class="info">
									<tr>
										<td 

style="border:none;">Nama Perusahaan</td>
										<td 

style="border:none;">: '.$data['vendor']['legal_name'].' <span>'.$data['vendor']['name'].'</td>
									</tr>
									<tr>
										<td 

style="border:none;">Alamat</td>
										<td 

style="border:none;">: '.$data['vendor']['vendor_address'].'</td>
									</tr>
								</table>
								'.$rekap_k3.'
								<p class="certificateClass">
									Bedasarkan hasil penilaian 

sementara, maka perusahaan ini tergolong dalam peringkat:
									<span><i>'.$data['get_csms']
...
'.default_date(date('d-m-Y')).',<br>
										Ketua Panitia 

Prakualifikasi CSMS<br>
...
_____________________</p>
						...
						<div class="page" style="margin: 0 auto;">
							<h2 align=center>HASIL PENILAIAN PRA 

KUALIFIKASI CSMS KONTRAKTOR(VERIFIKASI LAPANGAN)</h2>
							'.$csms_k3.'
						</div>';*/
		}
...
charset=utf-8">
					<!--<link type="text/css" rel="stylesheet" href="'.base_url
...
kanan.png);*/
		...
		// var_dump(libxml_use_internal_errors(true));
		 ...
		 $dompdf->stream("sertifikat - ".$data['vendor']['name'].".pdf",array('Attachment' => 
...
1));
	...
	function csms($id = ""){
		...
		$data['csms_file'] 		= $this->km->get_k3_all_data($data['vendor']['id'])

['csms_file'];
		$data['value_k3']		= $this->km->get_penilaian_value($data['vendor']

['id'],$data['csms_file']['id']);
...
['legal_name'].' <span>'.$data['vendor']['name'].'</span></h2>
											<table 
...
<td valign="top">'.$data['get_csms']['value'].'</td>
									...
									<p>Jakarta, '.default_date

(date('d-m-Y',strtotime($data['vendor']['first_date']))).'</p>
								</div>
							</div>

							<div class="certificateFooter">
								<div class="txt">
									<p id="left">Dicetak pada 

tanggal : '.date("d/m/Y, H:i:s").'</p>
									<p id="right">
										Dicetak oleh Sistem 

Aplikasi Kelogistikan PT Nusantara Regas.<br>
										Dokumen ini resmi 

tanpa stempel dan/atau tanda tangan pejabat.
									</p>
								</div>
							</div>
						</div>
...
PRAKUALIFIKASI DOKUMEN CSMS</h2>
							...
							<table class="info">
								<tr>
									<td style="border:none;">Nama 

Perusahaan</td>
									<td style="border:none;">: 

'.$data['vendor']['legal_name'].' <span>'.$data['vendor']['name'].'</td>
								</tr>
								<tr>
									<td 
...
'.$data['vendor']['vendor_address'].'</td>
								</tr>
							</table>
							'.$rekap_k3.'
							<p class="certificateClass">
								Bedasarkan hasil penilaian sementara, 

maka perusahaan ini tergolong dalam peringkat:
								<span><i>'.$data['get_csms']
...
CSMS<br>
					...
					<div class="page" style="margin: 0 auto;">
						<h2 align=center>HASIL PENILAIAN PRA KUALIFIKASI CSMS 

KONTRAKTOR(VERIFIKASI LAPANGAN)</h2>
						'.$csms_k3.'
					</div>
				</body>
			</html>';
