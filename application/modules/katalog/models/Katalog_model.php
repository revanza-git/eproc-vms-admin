...
class Katalog_model extends CI_Model{
	...
	public function get_katalog($category, $search='', $sort='', $page='', $per_page='',$is_page=FALSE,$filter=array()){
	...
	public function get_barang_compare($compare){
  ...
  if($this->input->post('id_procurement')){
	...
	public function get_harga($id_arr){
  ...
  return null;
	...
	public function edit_harga_barang($data,$id){
		$this->db->where('id',$id);
  ...
  return null;
	...
	public function save_harga_barang($id,$data){
		$_param = array();
	...
	public function data_chart($id){
		$this->get_data_barang($id);
	...
	public function get_data_compare($id){
		return $this->db->select('ms_material.*, tb_kurs.symbol,tb_kurs.name kurs_name ')->where_in('ms_material.id',$id)->join('tb_kurs','tb_kurs.id = ms_material.id_kurs','LEFT')->get('ms_material')->result_array();
	}

	public function data_chart_compare($id){
		$this->get_data_barang($id);
		$raw_data 	= $this->db->select(' YEAR(`date`) as years,(SELECT AVG(`price`) FROM `tr_material_price` WHERE id_material = '.$id.' AND del=0 AND YEAR(`date`)=years) as avg_year')
							->where('id_material',$id)
							->group_by('years')
