...
class json_provider_model extends CI_Model{
	...
	public function select_barang($id_barang = ''){
	...
	public function select_lelang($id_lelang = ''){
	...
	public function get_total_penawaran($id_barang = '', $id_vendor = ''){
	...
	public function get_initial_data($id_lelang = '', $id_barang = '', $is_update = false){
       ...
       $sql .= " ASC";
			...
			foreach($sql->result() as $data) 
				$_return[] = array('x' => $data->entry_stamp, 'y' => $data->nilai);
				
			$return[] = array('name' => $_peserta->name, 'data' => $_return);
		}
		
		return $return;
	}
	
	public function get_chart_update($id_lelang = ''){
  ...
  $this->db->where('id_procurement',$get_proc['id'])->where('del',0)->get('ms_procurement_barang')->result_array();
		...
		foreach($sql->result() as $data)
			$return[] = array('id' => $data->id, 'data' => $this->get_initial_data($id_lelang, $data->id, true));
	...
	public function get_same_offers($id_barang = '', $value = '', $id_vendor = ''){
		$ord = '';
  ...
  return 0;
	...
	public function get_latest_pos($id_barang = '', $ord = ''){
	...
	public function get_list($symbol = '', $ord = '', $id_barang = '', $id_vendor = '', $is_latest = false, $is_same = false){
  ...
  if ($is_latest) {
	...
	public function get_rank($id_lelang = '', $id_user = ''){	
      ...
      $symbol = "<";
       ...
       if ($hps < $latest) {
        ...
        $rank++;
			...
			$return[] = array('id' => $data->id, 'rank' => $rank);
		}
   ...
   return null;
	...
	{
		return $this->db->where('id_procurement',$id_lelang)->where('del',0)->get('ms_procurement_barang')->result_array();
	}
