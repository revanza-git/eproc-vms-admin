...
class K3_model extends CI_Model{
	...
	public function get_master_header(){
		...
		foreach($res as $re){
			$result[$re['id']] = $re['question'];
		}
  
		return $result;
	}
 
	public function get_sub_header(){
		$result = array();
	...
	public function get_quest(){
		...
		foreach($res as $re){
			$quest = $this->db->select('*')->where('id_question',$re['id'])->get('ms_answer_hse')->result_array();
			foreach($quest as $quest){
				$result[$re['id_ms_header']][$re['id_sub_header']][$re['id']][$quest['id']] = $quest;
			}
		}
	...
	public function get_csms($id_vendor){
		return $this->db->select('ms_csms.*, tb_csms_limit.value')->where('id_vendor',$id_vendor)->where('ms_csms.del',0)->join('tb_csms_limit','tb_csms_limit.id=ms_csms.id_csms_limit','LEFT')->order_by('ms_csms.id','desc')->get('ms_csms')->row_array();
	...
	public function save_k3_data($post,$id_user){
	...
	public function update_k3_data($post,$id_user){
		foreach($post as $id_question => $data){
	...
	public function save_csms_data($post,$id){
	...
	public function edit_csms_data($post,$id_user,$id){
		$id_csms_limit = $this->get_k3_limit($post['score']);
	...
	public function update_score($post,$id){
		$num_rows = $this->db->where('id_vendor',$id)->get('ms_score_k3')->num_rows();

		if ($num_rows==0) {
      return $this->db->insert('ms_score_k3',array(
   										'id_vendor'=>$id,
	...
	public function save_evaluasi_poin($post, $id, $act = 'create',$id_csms=0){
		$base_total = array();
	...
	public function get_history_nilai($id, $search='', $sort='', $page='', $per_page='',$is_page=FALSE,$filter=array()){
		$this->db->select('ms_score_k3.*, ms_vendor.name name')
		...
		return $this->db->get('ms_score_k3')->result_array();
	}
    ...
    public function get_header_list(){
		...
		foreach($ms_quest as $m_quest){
			$result[$m_quest['id']] = $m_quest['question'];
		}
	...
	public function get_quest_edit($id){
		return $this->db->select('*')->where('id',$id)->get('tb_quest')->result_array();
	}

	public function save_edit_header($data, $id){
    	$this->db->where('id',$id);
		
		return $this->db->update('tb_ms_quest_k3',$data);
    }

    public function save_edit_sub_quest($data, $id){
    	$this->db->where('id',$id);
		
		return $this->db->update('tb_sub_quest_k3',$data);
    }

    public function save_edit_quest($data, $id){
    	$this->db->where('id',$id);
		
		return $this->db->update('ms_answer_hse',$data);
    }

    public function save_edit_group($data, $id){
    	$this->db->where('id',$id);
		
		return $this->db->update('tb_quest',$data);
    }
	...
	public function reset_csms($id){
		$this->db->where('id_vendor',$id)->delete('tr_answer_hse');
		$this->db->where('id_vendor',$id)->delete('ms_csms');
		return true;
	}
	
	public function get_aspek_k3($id_vendor){
		return $this->db->where('id_vendor',$id_vendor)
					->get('ms_score_k3')
					->result_array();
	}
	
	public function get_klasifikasi(){
		return $this->db->get('tb_csms_limit')->result_array();
	}
	...
	public function get_vendor_data($id){
		return $this->db->select('ms_vendor.name name, tb_legal.name type')
