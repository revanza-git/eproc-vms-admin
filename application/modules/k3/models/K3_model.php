...
class K3_model extends CI_Model
{
	...
	function get_quest()
	{
		...
		foreach ($res as $key => $row) {
			$quest = $this->db->select('*')->where('del', 0)->where('id_question', $row['id'])->get('ms_answer_hse')->result_array();
	...
	{
		$quest = $this->db->select('*,ms_answer_hse.id id')
			->where('ms_answer_hse.del', 0)
			->where('tb_quest.del', 0)
		...
		foreach ($this->get_master_header() as $key_ms => $value_ms) {
			...
			foreach ($evaluasi as $key_ev => $data_ev) {
				...
				foreach ($quests as $key_quest => $quest) {
					...
					$answer = $this->db->where('id_question', $quest['id'])->where('del', 0)->get('ms_answer_hse')->result_array();
	...
	{
		$result = $this->db->select('*')->where('del', 0)->where('id', $id)->get('tb_quest')->result_array();
	...
	{
		$a = $this->db->where('id_vendor', $id_vendor)->get('ms_csms');
		return $a->row_array();
	}
