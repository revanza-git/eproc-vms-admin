...
class pemilik_model extends CI_Model{
	...
	public function edit_data($data,$id){
		...
		return $this->db->update('ms_pemilik',$data);
	}
	...
	public function get_pemilik_list($search='', $sort='', $page='', $per_page='',$is_page=FALSE,$filter=array()) 
    {
	...
	public function get_pemilik_admin_list($id) 
    {
    	$this->session->userdata('user');
		$this->db->select('*');
    ...
    public function get_total_percentage($id=0){
    	$user = $this->session->userdata('user');
	...
	{
		return $this->db->where('del', 0)->where('id_vendor', $id_vendor)->get('tr_surat_ubo')->row_array();
	}
}