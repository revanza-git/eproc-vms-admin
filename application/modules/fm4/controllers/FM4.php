...
class FM4 extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model('FM4_model', 'fm4');
        $this->load->library('utility');
    }
    public function index()
    {
        $user = $this->session->userdata('user');
        $data_k3['csms_file']     = $this->fm4->get_k3_all_data($user['id_user']);

        if (empty($data_k3['csms_file']['csms_file']) && empty($data_k3['csms_file']['answer'])) {
            redirect('fm4/first_form');
        }


        $data_k3['data_k3']            = $this->fm4->get_k3_data($user['id_user']);
        $data_k3['field_quest']     = $this->fm4->get_field_quest();
        $data_k3['ms_quest']        = $this->fm4->get_header_list();
        $data_k3['sub_quest']        = $this->fm4->get_sub_quest_list();
        $data_k3['data_quest']        = $this->fm4->get_quest_list();
        $data_k3['data_field']        = $this->fm4->get_data_field();
    ...
    {
        $user = $this->session->userdata('user');
        $res = $this->fm4->reset_csms($this->session->userdata('user')['id_user']);
        if ($res) {
        ...
        if ($this->input->post('next')) {
            ...
            if ($this->input->post('csms') == 1) {
                ...
                if ($this->form_validation->run() == TRUE) {
                    ...
                    $res = $this->fm4->save_csms_data($this->input->post(), $user['id_user']);
                ...
                redirect(site_url('fm4/csms_form'));
            }
        }
    ...
    {
        $user = $this->session->userdata('user');
        $data     = $this->fm4->get_k3_all_data($user['id_user'])['csms_file'];
        ...
        if ($this->input->post('next')) {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                $res = $this->fm4->edit_csms_data($this->input->post(), $user['id_user'], $data['id']);
    ...
    {
        $user = $this->session->userdata('user');
        /*$data['ms_quest']		= $this->fm4->get_master_header();$data['sub_quest']		= $this->fm4->get_sub_header();$data['quest']			= $this->fm4->get_quest();$data['field_quest'] 	= $this->fm4->get_field_quest();*/
        $data['data_k3']        = $this->fm4->get_k3_data($user['id_user']);
        $data['csms_file']         = $this->fm4->get_k3_all_data($user['id_user']);
        $data['field_quest']     = $this->fm4->get_field_quest(); // pertanyaan sub 2
        $data['ms_quest']        = $this->fm4->get_header_list(); // header bagian
        $data['sub_quest']        = $this->fm4->get_sub_quest_list(); // sub list 2.1
        $data['data_quest']        = $this->fm4->get_quest_list(); // border square
        $data['data_field']        = $this->fm4->get_data_field(); // question
        $data['quest_all']        = array();
        $vendor                    = $this->fm4->get_vendor_data($user['id_user']);
        ...
        if ($this->input->post('simpan')) {
            // foreach($this->fm4->get_admin_email(3) as $key => $admin){
            ...
            $res = $this->fm4->save_k3_data($this->input->post('quest'), $user['id_user']);
            if ($res) {
        ...
        // echo print_r($this->fm4->get_k3_data($user['id_user']));
        if (count($this->fm4->get_k3_data($user['id_user'])) > 0) {
    ...
    {
        $user = $this->session->userdata('user');
        $data['ms_quest']        = $this->fm4->get_master_header();
        $data['sub_quest']        = $this->fm4->get_sub_header();
        $data['quest']            = $this->fm4->get_quest();
        $data['field_quest']     = $this->fm4->get_field_quest();
        $data['data_k3']        = $this->fm4->get_k3_data($user['id_user']);
        $vendor                    = $this->fm4->get_vendor_data($user['id_user']);
        // print_r($this->fm4->get_admin_email(3));die;
        ...
        if ($this->input->post('simpan')) {
            ...
            $res = $this->fm4->update_k3_data($this->input->post('quest'), $user['id_user'], 'edit');
            if ($res) {
    ...
    public function get_vendor_group()
    {
        ...
        $data['vendor_list'] = $this->fm4->get_k3_vendor($search, $sort, $page, $per_page, TRUE);

        // $data['filter_list'] = $this->form->group_filter_post($this->get_field());

        $data['pagination'] = $this->utility->generate_page('fm4/get_vendor_group', $sort, $per_page, $this->fm4->get_k3_vendor($search, $sort, '', '', FALSE));
        $data['sort'] = $sort;
        $data['admin'] = $admin;
    ...
    public function history_nilai($id)
    {
        ...
        $data['pagination'] = $this->utility->generate_page('admin/admin_dpt', $sort, $per_page, $this->fm4->get_history_nilai($id, $sort, '', '', FALSE));
        $data['sort'] = $sort;
        $data['history'] = $this->fm4->get_history_nilai($id);
    ...
    public function penilaian_k3($id_vendor, $act = 'create', $id_csms = 0)
    {
        ...
        $data['quest'] = $this->fm4->get_quest();
        $data['field_quest'] = $this->fm4->get_field_quest();
        $data['evaluasi_list'] = $this->fm4->get_evaluasi_list();
        $data['evaluasi'] = $this->fm4->get_evaluasi();
        $data['data_k3'] = $this->fm4->get_k3_data($id_vendor);
        $data['get_csms'] = $this->fm4->get_csms($id_vendor);
        $data['get_hse'] = $this->fm4->get_hse($id_vendor);

        $data['csms_file']     = $this->fm4->get_k3_all_data($id_vendor)['csms_file'];

        $data['value_k3'] = $this->fm4->get_penilaian_value($id_vendor, $data['csms_file']['id']);
        $vendor                    = $this->fm4->get_vendor_data($id_vendor);

        // print_r($data['evaluasi']);die;
        if ($this->input->post('simpan')) {

            $res = $this->fm4->save_evaluasi_poin($this->input->post('evaluasi'), $id_vendor, $act, $id_csms);
            ...
            if ($res) {
                ...
                $email['message']    = 'Admin Logistik telah selesai mengadakan penilaian CSMS untuk Penyedia Barang / Jasa ' . $vendor['type'] . ". " . $vendor['name'] . "  dalam Sistem Aplikasi Kelogistikan PT Nusantara Regas.
										...
										Terimakasih,<br>
										PT Nusantara Regas";
                foreach ($this->fm4->get_admin_email(1) as $key => $admin) {
                    $this->utility->mail($admin['email'], $email['message'], $email['subject']);
                }
                $this->session->set_flashdata('msgSuccess', '<p class="msgSuccess">Sukses menyimpan data!</p>');
    ...
    public function penilaian_view($id_vendor)
    {
        ...
        $data['data_k3'] = $this->fm4->get_k3_data($id_vendor);
        $data['csms_limit'] = $this->fm4->get_csms_limit($id_vendor);



        $data['data_poin'] = $this->fm4->get_poin($id_vendor);
        $data['csms_file']     = $this->fm4->get_k3_all_data($id_vendor)['csms_file'];
    ...
    {
        $admin = $this->session->userdata('admin');
        $data     = $this->fm4->select_csms($id_vendor);
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            $res = $this->fm4->edit_csms_score($id_vendor, $_POST['score']);

            if ($res) {
                $this->session->set_flashdata('msgSuccess', '<p class="msgSuccess">Sukses mengubah data!</p>');
                redirect(site_url('fm4/get_vendor_group'));
            }
        }
    ...
    {
        $data = $this->fm4->get_k3_vendor($search, $sort, $page, $per_page, TRUE);
        // print_r($data);die;	
